...
class Strategy2 extends Strategy{
    ...
    choice(){
        ...
        console.log("This is visible Inside Strat2 " + this.faceUpCards)
        for(var i = 0; i < this.faceUpCards.length; i++){
            if(this.faceUpCards[i].length < 3){
                continue
            }
            for(var a = 0; a < this.faceUpCards[i].length-2; a++){
                if((this.faceUpCards[i][j].number === this.faceUpCards[i][j+1].number) && (this.faceUpCards[i][j].number === this.faceUpCards[i][j+2].number)){
